<script>
    ...
    export let secondaryColor = 'pink';
    $: classes = `${bgColor} ${secondaryColor}`
</script>
...
{#if secondaryColor === 'pink'}
    {#if newStory}
        <form action="{action}" class="{classes}">
...
{:else if secondaryColor === 'yellow'}
    {#if newStory}
    <form action="{action}" class="{classes}">
        <h2 class="form-title-yellow">Create your story!</h2>
        <div class="form-control">
            <input type="text" id="title" name="title" placeholder="Enter your story title here">
        </div>
        <div class="form-control">
    ...
    {:else}
    <form action="{action}" class="{classes}">
    <h2 class="form-title-yellow">Create another page!</h2>
    <div class="form-control">
        <textarea name="img-description" id="img-description" placeholder="Describe the image you would like to see! Example, friends playing soccer outiside on a sunny day!"></textarea>
    </div>
    <div class="form-control">
